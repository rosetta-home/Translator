...
class BarChart extends Component {
  ...
  /* Renders the component */
    ...
    if (enable) {
      return ( <div><NVD3Chart id="barchart" showXAxis={enable} showYAxis={enable} height={400} color={this.handleColor} type="discreteBarChart" datum={payload} x="x" y="y" xAxis={{ tickFormat: (d) => d3.time.format('%Y-%m-%d')(new Date(d)), ticks:6,rotateLabels: -35  }}/></div> );
    } else {
      return ( <div><NVD3Chart margin={{top: 0, right: 10, bottom: 0, left: 10}} id="barchart" height={400} showXAxis={enable} showYAxis={enable} color={this.handleColor} type="discreteBarChart" datum={payload} x="x" y="y" xAxis={{ tickFormat: (d) => d3.time.format('%Y-%m-%d %H:%M:%S %p')(new Date(d)), ticks:6,rotateLabels: -35  }}/></div> );
    }
  }
}
